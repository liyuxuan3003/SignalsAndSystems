...
\begin{Proof}
    ...
    这里可以将$x(t)=1$视为一个双边偶指数函数的极限，如\xref{tab:常值函数的傅里叶变换}所示
    \begin{Equation}
        x(t)=\Lim[a\to 0]\e^{-a|t|}=1
    \end{Equation}
    故可以预期$x(t)$的傅里叶变换$X(\j\omega)$也是双边偶指数函数的极限，依据\xref{exp:双边偶指数函数的傅里叶变换}
    \begin{Equation}
        X(\j\omega)=\Lim[a\to 0]\frac{2a}{a^2+\omega^2}
    \end{Equation}
    然而这个极限是发散的，为此，将两边对$\omega$积分
    \begin{Equation}
        \qquad\qquad\qquad
        \Int[-\infty][\infty]X(\j\omega)\dd{\omega}=\Lim[a\to 0]\Int[-\infty][\infty]\frac{2a}{a^2+\omega^2}\dd{\omega}=\Lim[a\to\infty]2\eval{\arctan\frac{\omega}{a}}_{-\infty}^{+\infty}=2\pi
        \qquad\qquad\qquad
    \end{Equation}
    ...
    \begin{Equation}*
        X(\j\omega)=2\pi\dirac(\omega)
    \end{Equation}
    很巧合的是，以上过程其实也顺带证明了\xref{tab:冲激函数的其他辅助函数}曾给出的一个冲激函数的辅助函数。
\end{Proof}
...
\begin{Proof}
    ...
    \end{Equation}
    这里可以将$x(t)=\sgn(x)$视为一个双边奇指数函数的极限，如\xref{tab:符号函数的傅里叶变换}所示
    \begin{Equation}
        x(t)=\Lim[a\to 0]\e^{-\alpha|t|}\sgn(t)=1
    \end{Equation}
    故可以预期$x(t)$的傅里叶变换$X(\j\omega)$也是双边奇指数函数的极限，依据\xref{exp:双边奇指数函数的傅里叶变换}
    \begin{Equation}
        X(\j\omega)=\Lim[a\to 0]\frac{-2\j\omega}{a^2+\omega^2}
    \end{Equation}
    这个极限的计算没有任何难度
    \begin{Equation}*
        X(\j\omega)=\frac{-2\j}{\omega}=\frac{2}{\j\omega}\qedhere
    \end{Equation}
\end{Proof}
