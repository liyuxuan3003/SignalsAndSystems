...
\xgp[4pt][-10pt]{$\mal{\e^{\alpha t}\Sum[k=0][r-1]P_k't^k\cos(\beta t)+Q_k't^k\sin(\beta t)}$}&$\lambda=\alpha\pm\j\beta$是$s$重共轭复根&\xgp[4pt][-10pt]{$\mal{\e^{\alpha t}x^s\Sum[k=0][r-1]P_kt^k\cos(\beta t)+Q_kt^k\sin(\beta t)}$}\\
\end{Tablex}
